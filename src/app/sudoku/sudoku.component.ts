...
export class SudokuComponent implements OnInit {
  ...
  public checkProper(r:number,c:number, newNum: number){
    ...
    let row = ((r/3)>>0)*3, col = ((c/3)>>0)*3;

    for(let i = row; i < row+3; i++){
      for(let j= col; j < col+3; j++){
        if(i != r && j != c && this.answer[i][j] && this.answer[i][j] == newNum)
        return false;
      }
    }
  ...
  public getRandomGenerator(){
    ...
    return function(){
      pointer = (pointer + 1 + (Math.random() * sOption.length)>>0) % sOption.length;
  ...
  public resetSoduku(){
    ...
    for(let i = 0; i< this.size; i++){
      for(let j = 0; j< this.size; j++){
          this.puzzle[i][j] = this.prefilled.get(i * this.size + j) ?   this.answer[i][j] : '';
      }
    }
  }
